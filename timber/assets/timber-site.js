  ...
  (function ( app, undefined ) {
    ...
    var timber = {
      ...
      docsNav: function() {
        ...
        var docsNavWrapper = $('.docs-nav'),
            gettingstartedNav = $('#gettingstartedNav'),
            basicNav = $('#basicNav'),
            styleguideNav = $('#styleguideNav'),
            moduleNav = $('#moduleNav'),
            jsModuleNav = $('#jsModuleNav');

        // Generate getting started links
        $('.gettingstarted-title').each(function() {
          var el = $(this),
              title = el.html(),
              hash = el.attr('id');
          gettingstartedNav.append('<li><a href="#'+ hash +'" class="doc-nav-link">'+ title +'</a></li>');
        });
        ...
        $('.grid-title').each(function() {
          var el = $(this),
              title = el.html(),
              hash = el.attr('id');
          basicNav.append('<li><a href="#'+ hash +'" class="doc-nav-link">'+ title +'</a></li>');
        });

        // Generate style guide links
        $('.styleguide-title').each(function() {
          var el = $(this),
              title = el.html(),
              hash = el.attr('id');
          styleguideNav.append('<li><a href="#'+ hash +'" class="doc-nav-link">'+ title +'</a></li>');
        });
        ...
        $('.module-title').each(function() {
          var el = $(this),
              title = el.html(),
              hash = el.attr('id');
          moduleNav.append('<li><a href="#'+ hash +'" class="doc-nav-link">'+ title +'</a></li>');
        });

        // Generate javascript module links
        $('.js-title').each(function() {
          var el = $(this),
              title = el.html(),
              hash = el.attr('id');
          jsModuleNav.append('<li><a href="#'+ hash +'" class="doc-nav-link">'+ title +'</a></li>');
        });
      ...
      docsHelpers: function() {
        ...
        // Add titles to each .grid-demo
        $('.grid-demo').each(function() {
          var el = $(this)
              parentClass = el.parent().attr('class');
           el.attr('title', parentClass);
        });
      },

      waypoints: function() {
        var anchors = $('.doc-title'),
            navLinks = $('.doc-nav-link'),
            navSections = $('.docs-nav > ul');

        anchors.waypoint(function() {
          var currentId = $(this).attr('id'),
              activeLink = $('.doc-nav-link[href="#'+ currentId +'"]');

          // Set active link
          navLinks.removeClass('current-docs');
          activeLink.addClass('current-docs');

          // Set active parent list
          navSections.removeClass('current-section');
          activeLink.closest('ul').addClass('current-section');
        }, {offset: 5});

        navLinks.on('click', function() {
          currentLink = $(this);

          // Slight delay to avoid waypoint overriding the click
          setTimeout(function() {
            navLinks.removeClass('current-docs');
            currentLink.addClass('current-docs');
          }, 50);
        });
      },
